...
Die Eigenschaften eines Algorithmus sind also dass

1. er in einem endlichen Folge von Anweisungen (Text) eindeutig beschreibbar ist (Unmissverständlichkeit und Endlichkeit),
2. jede Anweisung auch ausführbar ist (Ausführbarkeit),
3. er zu jedem Zeitpunkt nur endlich viel Speicher benötigt (dynamisch endlicher Platzbedarf),
4. er nur endlich viele Schritte benötigt (Terminierung)

oftmals wird zudem erwartet, dass

6. er für die gleiche Eingabe auch die gleiche Ausgabe liefert (Derterminiertheit)
7. und zu jedem Zeitpunkt die nächste anzuwendende Anweisung eindeutig definiert ist (Determinismus).
