...
2. Notieren Sie den Prozess, sodass ein kleines Kind Ihren [Algorithmus](def-algorithm) nachvollziehen und ausführen kann.
3. Lassen Sie Ihren notierten Algorithmus von einer anderen Person ausführen.
...
Stets befinden wir uns in einem bestimmten *Kontext*.
...
Geht dies nicht hervor, müssen wir mit den Mitteln die uns zur Verfügung stehen, d. h., mit dem bestehenden Kontext ausdrücken, **wie** **"Sortiere Karten"** realisiert wird.
Die Unterscheidung zwischen dem **Wie** und dem **Was** ist essenziell.
Der Kontext beschreibt viele verschiedene **Was**. 
Mit diesen beschreiben wir **wie** wir ein anderes **Was** realisieren.
Wir erweitern den Kontext um jenes **Was**, in unserem Fall um **"Sortiere Karten"**.
...
Gehen wir davon aus, dass **"Sortiere Karten"** in Ihrem Kontext nicht definiert ist.
...
Was noch zur Verwirrung führen kann ist der unspezifizierte Stapel, d.h.,
...
Welche Eigenschaften hat ein Stapel?
...
Die Sprünge (``Springe zu Zeile``) sind hierbei durch ``Solange`` und die Einrückung definiert.
...
``S`` ist dabei der Stapel auf den wir die kleinste Karte ``s`` legen.
...
meinen wir in unserem *Kontext*, dass der Name (die Variable) ``x`` der das Objekt ``o1`` (hier eine Karte) identifiziert, nach der Anweisung, das Objekt ``o2`` identifiziert, welches derzeit durch den Namen ``y`` identifiziert wird.
...
Das mathematische Denken ist eng mit dem [Computational Thinking](sec-what-is-ct) verknüpft.
In der Mathematik sind alle Objekte und Operationen präzise und unmissverständlich formuliert.
Zugleich verzichtet man auf überflüssige Informationen und versucht dadurch Definitionen allgemein bzw. abstrakt zu halten.
...
Schon sehr früh können wir Objekte ihrer Größe, Länge oder ihres Gewichts nach sortieren.
...
Für jedes Tupel $(a, b)$ der Relation $R$ gilt: $a \leq b$.
